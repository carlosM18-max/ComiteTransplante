  ...
  export default {
    ...
    methods: {
      // Método para cambiar la pestaña actual del wizard
      changeTab(val) {
        this.currentindex = val
      },
      // Método para manejar el envío del formulario
      onSubmit() {
        // Simula un tiempo de espera de 3 segundos antes de redirigir
        setTimeout(() => {
          // Redirige a la tabla básica después de 3 segundos
          this.$router.replace('/table/tables-basic');
        }, 3000);
      },
      // Método para validar si un campo es obligatorio
      isRequire(value) {
        if (value && value.trim()) {
          return true
        }
